...
use schema::{
    decode_components, encode_components, hit_key, parse_placement_field, placement_field,
    placement_key, worker_blocks_key, worker_field_prefix, worker_meta_key, TOKEN_COUNT_FIELD,
};
...
/// One hash's REPLACE report at a single tier: the pre-encoded component set and
/// the block's token count (`0` when unknown / legacy).
struct ReportOne<'a> {
    hash: &'a str,
    components: String,
    token_count: u32,
...
impl RedisKvIndexerBackend {
    ...
    async fn apply(
        ...
        // self-heals across reconnects). Only written when supplied.
        if let Some(spec) = &req.cache_spec {
            self.set_worker_spec(worker, spec).await?;
        }
        ...
        for action in &req.actions {
            ...
            match ExternalKvActionType::try_from(action.r#type) {
                Ok(ExternalKvActionType::ActionReport) => {
                    ...
                    // empty for a legacy whole-block store.
                    let reports: Vec<ReportOne> = action
                        .hashes
                        .iter()
                        .enumerate()
                        .map(|(index, hash)| ReportOne {
                            hash: hash.as_str(),
                            components: encode_components(
                                action
                                    .component_sets
                                    .get(index)
                                    .map(|set| set.components.as_slice())
                                    .unwrap_or(&[]),
                            ),
    ...
    async fn do_match(
        ...
        for (hash, flat) in hashes.iter().zip(per_hash) {
            ...
            for (worker, tier, _components) in placement.entries {
    ...
    async fn placement_holders(&self, hash: &str) -> Result<Vec<String>, Status> {
        ...
        for (worker, _tier, _components) in parse_hash_placement(&flat).entries {
    ...
    async fn collect_prefix_inputs(
        ...
        for (index, flat) in per_hash.iter().enumerate() {
            let placement = parse_hash_placement(flat);
            let mut per_worker: HashMap<String, Vec<(i32, Vec<String>)>> = HashMap::new();
            for (worker, tier, components) in placement.entries {
                per_worker
                    .entry(worker)
                    .or_default()
                    .push((tier, components));
            }
            for (worker, tier_components) in per_worker {
...
struct HashPlacement {
    /// One `(worker, tier, components)` per placement field.
    entries: Vec<(String, i32, Vec<String>)>,
...
fn parse_hash_placement(flat: &[String]) -> HashPlacement {
    ...
    for pair in flat.chunks(2) {
        ...
        if field == TOKEN_COUNT_FIELD {
            ...
            entries.push((worker, tier, decode_components(value)));
        }
    }
    HashPlacement {
        entries,
