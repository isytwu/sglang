...
class KVCacheEventMixin:
    def _component_types_for_page(
        ...
        The base implementation returns ``None`` (legacy behaviour) so caches
        with a single, undifferentiated KV component keep byte-identical wire
        output. Component-aware caches (e.g. the unified radix tree) override
        this to report FULL / SWA / MAMBA placement per tier.
        """
        return None
    ...
    def _record_remove_event(self, node: Any, medium=None, component_types=None):
        ...
        # component is gone); ``component_types`` is informational and normally
        # left ``None``.
        if self.enable_kv_cache_events:
            if medium is None:
                medium = StorageMedium.GPU
