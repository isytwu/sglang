...
class UnifiedRadixCache(KVCacheEventMixin, BasePrefixCache):
    def __init__(
        ...
        self.enable_kv_events_component_types = params.enable_kv_events_component_types
        self.kv_event_queue = []
    ...
    def _reset_full(self) -> None:
        ...
        self._pending_store_event_nodes: Optional[list[UnifiedTreeNode]] = None
    ...
    # Maps the cache's internal component enum to the canonical on-wire names
    # (kv_events.KV_COMPONENT_*), so the event schema stays the single source of
    # truth for the component vocabulary rather than relying on ComponentType's
    ...
    def _note_insert_store_node(self, node: UnifiedTreeNode) -> None:
        ...
        """
        if self._pending_store_event_nodes is not None:
            self._pending_store_event_nodes.append(node)
    ...
    def _flush_insert_store_events(
        ...
        (``self._pending_store_event_nodes``) and (b) the new-suffix chain from the leaf
        ...
        for n in self._pending_store_event_nodes or ():
            _add(n)
        if is_new_leaf:
    ...
    def _insert_helper(
        ...
        # the save/restore is purely defensive. What is NOT optional is resetting
        ...
        try:
            ...
            self._pending_store_event_nodes = saved_pending_store_event_nodes
