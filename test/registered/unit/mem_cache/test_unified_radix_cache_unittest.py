...
def build_fixture(
    ...
    *,
    ...
    enable_kv_events_component_types: bool = True,
):
    """Create (tree, allocator, req_to_token_pool) from a CacheConfig.

    ``enable_kv_events_component_types`` defaults to ``True`` so the
    component-placement tests exercise the component-aware path; the production
    default is ``False``.
    """
...
class TestUnifiedRadixCacheKVEvents(CustomTestCase):
    ...
    def test_full_only_store_events_omit_component_types(self):
        ...
        removed = self._removed_events(cache, StorageMedium.GPU)
        self.assertEqual(len(removed), 1)
        # BlockRemoved is whole-block by construction and carries no component
        # dimension (the field was intentionally not added to BlockRemoved).
        self.assertFalse(hasattr(removed[0], "component_types"))
...
class TestUnifiedRadixCacheComponentPlacementEvents(CustomTestCase):
    ...
    def test_component_types_disabled_returns_none(self):
        # With the enable_kv_events_component_types switch off, even a
        # multi-component tree omits the component dimension (the default wire
        # behaviour).
        cfg = CacheConfig(
            page_size=1,
